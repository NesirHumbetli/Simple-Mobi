...
if($_SERVER['REQUEST_METHOD']=="POST"){
    if(isset($_POST['delete-cart'])){
        $del = $Cart->deleteCart($_POST['item_id']);
    }
                        ...
                        <div class="row border-top py-3 mt-3">
                                ...
                                <div class="qty d-flex pt-2">
                                    <div class="d-flex font-rale w-25">
                                        <button class="qty-up border bg-light"
                                                data-id="<?= $item['item_id'] ?? 'Null'; ?>"><i
                                                    class="fas fa-angle-up"></i></button>
                                        <input type="text" data-id="<?= $item['item_id'] ?? 'Null'; ?>"
                                               class="qty_input border px-2 w-100 bg-light"
                                               disabled value="1" placeholder="1">
                                        <button data-id="<?= $item['item_id'] ?? 'Null'; ?>"
                                                class="qty-down border bg-light"><i
                                                    class="fas fa-angle-down"></i></button>
                                    </div>
                                    <form method="post">
                                        <input type="hidden" name="item_id" value="<?=$item['item_id']??0;?>">
                                    <button type="submit" name="delete-cart" class="btn font-baloo text-danger px-3 border-right">Delete
                                    </button>
                                    </form>
                            ...
                            <div class="col-sm-2 text-right">
                                <div class="font-size-20 text-danger font-baloo">
                                    $<span class="product_price" data-id="<?= $item['item_id'] ?? 'Null'; ?>"><?= $item['item_price'] ?? 0; ?></span>
                                </div>
                            </div>
                        </div>
                        <?php
